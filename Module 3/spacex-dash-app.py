...
mean_payload = round(spacex_df['Payload Mass (kg)'].min(), 0)
...
def payload_mass(site, payload):
    filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= payload[0]) & (spacex_df['Payload Mass (kg)'] <= payload[1])]
    if site == 'ALL':
        fig = px.scatter(filtered_df, x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return fig
    else:
        fig = px.scatter(filtered_df[filtered_df['Launch Site'] == site], x="Payload Mass (kg)", y="class", color="Booster Version Category")
        return fig
